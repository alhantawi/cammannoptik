...
export const ProtocolStack: React.FC = () => {
  ...
  const renderGraphic = (type: "radial" | "scan" | "wave") => {
    if (type === "radial") {
      return (
        <svg
          viewBox="0 0 100 100"
          className="w-full h-full animate-[spin_25s_linear_infinite] opacity-80 stroke-[#161719] dark:stroke-white/40"
          fill="none"
          strokeWidth="0.5"
        >
    ...
    if (type === "scan") {
      return (
        <div className="relative w-full h-full overflow-hidden rounded-2xl bg-white/60 dark:bg-white/5 border border-[#161719]/10 dark:border-white/10 flex items-center justify-center p-4 shadow-inner">
          <svg viewBox="0 0 100 100" className="w-full h-full opacity-35 dark:opacity-50 stroke-[#161719] dark:stroke-white/40" fill="none" strokeWidth="0.5">
            <defs>
              <pattern id="lens-grid-cammann" width="10" height="10" patternUnits="userSpaceOnUse">
                <path d="M 10 0 L 0 0 0 10" fill="none" className="stroke-[#161719] dark:stroke-white/40" strokeWidth="0.5" />
              </pattern>
            </defs>
            <rect width="100" height="100" fill="url(#lens-grid-cammann)" />
            <circle cx="50" cy="50" r="32" className="fill-[#FAF8F5] dark:fill-[#202226] stroke-[#161719] dark:stroke-white/40" strokeWidth="0.8" />
    ...
    return (
      <div className="relative w-full h-full flex items-center justify-center p-4 bg-white/60 dark:bg-white/5 rounded-2xl border border-[#161719]/10 dark:border-white/10 overflow-hidden shadow-inner">
        ...
        <svg viewBox="0 0 120 100" className="w-full h-full fill-none relative z-10" strokeLinecap="round" strokeLinejoin="round">
          ...
          <path
            ...
            className="stroke-[#161719] dark:stroke-white/30"
            strokeWidth="1"
            opacity="0.2"
          />
        ...
        <div className="absolute bottom-3 left-4 flex items-center gap-1.5 font-mono text-[9px] text-[#D13426] tracking-wider font-semibold bg-white/80 dark:bg-[#161719]/80 px-2 py-0.5 rounded-full border border-[#D13426]/20">
  ...
  return (
    <section id="leistungen" ref={containerRef} className="bg-[#FAF8F5] dark:bg-[#161719] py-24 relative transition-colors duration-500">
      {/* Heading */}
      <div className="text-center mb-12 px-6 max-w-3xl mx-auto">
        ...
        <h2 className="font-outfit text-3xl sm:text-4xl md:text-5xl font-black mb-3 text-[#161719] dark:text-white">
          Unsere Leistungen im Überblick
        </h2>
        <p className="font-serif italic text-2xl text-[#D13426]">
          Brillengläser, Kontaktlinsen & meisterhafter Werkstattservice
        </p>
      </div>
      ...
      <div className="relative max-w-6xl mx-auto px-4 sm:px-6">
        {protocolCards.map((card) => (
          ...
          >
            <div className="w-full bg-white dark:bg-[#202226] rounded-3xl md:rounded-[2.5rem] lg:rounded-[3rem] shadow-2xl overflow-hidden flex flex-col md:flex-row border border-[#161719]/10 dark:border-white/10 transition-colors duration-500">
              
              {/* Content Side */}
              <div className="flex-1 p-6 sm:p-8 md:p-10 lg:p-12 xl:p-14 flex flex-col justify-center">
                <div className="flex items-center justify-between mb-3">
                  <span className="font-mono text-3xl sm:text-4xl md:text-5xl font-light text-[#161719]/20 dark:text-white/20">
                    {card.id}
                  </span>
                ...
                <h4 className="font-mono text-xs sm:text-sm uppercase tracking-widest text-[#161719]/70 dark:text-white/70 font-semibold mb-1.5">
                  {card.subtitle}
                </h4>

                <h3 className="font-outfit text-2xl sm:text-3xl md:text-4xl font-black text-[#161719] dark:text-white mb-3 sm:mb-4 leading-tight">
                  {card.title}
                </h3>

                <p className="text-xs sm:text-sm md:text-base text-[#161719]/75 dark:text-white/75 leading-relaxed font-light mb-4 sm:mb-6">
                  {card.desc}
                </p>
                ...
                <div className="space-y-2 sm:space-y-2.5 mb-6 sm:mb-8">
                  {card.features.map((feature, i) => (
                    <div key={i} className="flex items-center gap-2.5 text-xs sm:text-sm text-[#161719]/85 dark:text-white/90 font-medium">
                ...
                <div className="pt-1">
                  <button
                    ...
                    className="luxury-btn px-6 py-3 bg-[#161719] dark:bg-white text-white dark:text-[#161719] hover:bg-[#232529] dark:hover:bg-[#FAF8F5] rounded-full text-xs font-semibold uppercase tracking-wider flex items-center gap-2 cursor-pointer shadow-md transition-all hover:scale-[1.02] active:scale-[0.98]"
                  >
                    <span>
                      {card.id === "01"
                        ? "Brillenglas-Beratung buchen"
                        : card.id === "02"
                        ? "Kontaktlinsen-Termin buchen"
              ...
              {/* Graphic Side */}
              <div className="flex-1 bg-[#FAF8F5]/80 dark:bg-[#161719]/80 relative flex items-center justify-center p-6 sm:p-8 md:p-12 overflow-hidden border-t md:border-t-0 md:border-l border-[#161719]/10 dark:border-white/10 min-h-[220px] md:min-h-0">
                <div className="absolute top-5 right-5 sm:top-6 sm:right-6 flex items-center gap-2 bg-white dark:bg-[#202226] px-3 py-1 rounded-full border border-[#161719]/10 dark:border-white/10 text-[10px] font-mono text-[#161719] dark:text-white shadow-sm">
